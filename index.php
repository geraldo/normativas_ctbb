...
if(isset($_GET['operation'])) {
	$fs = new fs(dirname(__FILE__) . DIRECTORY_SEPARATOR. 'data' . DIRECTORY_SEPARATOR . "POUM Castellbisbal (aprovació definitiva)" . DIRECTORY_SEPARATOR);
	//$fs = new fs("/var/www/html/ctbb/normativasCTBB/POUM Castellbisbal (aprovació definitiva)");
	//$fs = new fs("/var/servers/ctbb/web/normativasCTBB/POUM Castellbisbal (aprovació definitiva)");
	try {
		$rslt = null;
						...
						$.get('?operation=get_content&id=' + data.selected.join(':'), function (d) {
							if(d && typeof d.type !== 'undefined') {
								...
								switch(d.type) {
									case 'pdf':
										$('#data .pdf').show();
										//var basedir = '/var/www/html/ctbb/normativasCTBB/';
										var basedir = '/var/www/mapa/ctbb/normativas/';
							...
							if (data.node.id.indexOf("."+d.type) != -1) {
								...
								if (path.length == 2) {
									var f0 = "data/POUM Castellbisbal (aprovació definitiva)/";
									var f1 = path[0];
